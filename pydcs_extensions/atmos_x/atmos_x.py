"""Cloud presets shipped with the ATMOS X mod (v1.2).

Generated from the mod's ``Config/Effects/clouds.lua``. ``name`` must match the
preset key DCS uses in that file exactly -- it is what gets written to the .miz.
"""
...
from pydcs_extensions.cloud_injector import (
    eject_cloud_presets,
    inject_cloud_presets,
)
...
class AtmosXClouds(Enum):
    ...
    AltostratusAltocumulus1 = CloudPreset(
        name="Preset85",
        ui_name="Altostratus & Altocumulus 1 [ATMOS-X]",
    ...
    AltostratusAltocumulus2 = CloudPreset(
        name="Preset88",
        ui_name="Altostratus & Altocumulus 2 [ATMOS-X]",
    ...
    @staticmethod
    def activate() -> None:
        inject_cloud_presets(ATMOS_X_CLOUDS, ATMOS_X_PRECIPITATION_PRESETS)

    @staticmethod
    def deactivate() -> None:
        eject_cloud_presets(ATMOS_X_CLOUDS, ATMOS_X_PRECIPITATION_PRESETS)
...
ATMOS_X_CLOUDS = {
    ...
    "Preset85": AtmosXClouds.AltostratusAltocumulus1,
    "Preset86": AtmosXClouds.FewAltocumulus,
    "Preset87": AtmosXClouds.Altocumulus3layers,
    "Preset88": AtmosXClouds.AltostratusAltocumulus2,
}

#: Presets with a positive precipitationPower, i.e. the ones that actually rain.
ATMOS_X_PRECIPITATION_PRESETS = {
    "Preset35",  # Scattered Showers 1 [ATMOS-X], precipitationPower 0.3
    "Preset36",  # Scattered Showers 2 [ATMOS-X], precipitationPower 0.3
    "Preset37",  # Scattered Showers 3 [ATMOS-X], precipitationPower 0.6
    "Preset38",  # Scattered Showers 4 [ATMOS-X], precipitationPower 0.3
    "Preset39",  # Scattered Showers 5 [ATMOS-X], precipitationPower 0.4
    "Preset40",  # Scattered Showers 6 [ATMOS-X], precipitationPower 0.1
    "Preset41",  # Overcast Light Rain 1 [ATMOS-X], precipitationPower 0.1
    "Preset42",  # Overcast Light Rain 2 [ATMOS-X], precipitationPower 0.1
    "Preset43",  # Overcast And Moderate Rain 1 [ATMOS-X], precipitationPower 0.8
    "Preset49",  # Overcast Cumulus Light Mist [ATMOS-X], precipitationPower 0.04
    "Preset65",  # Nimbostratus with heavy rain [ATMOS-X], precipitationPower 0.8
    "Preset73",  # Scattered Thunderstorms 1 [ATMOS-X], precipitationPower 0.9
    "Preset74",  # Scattered Thunderstorms 2 [ATMOS-X], precipitationPower 0.9
    "Preset83",  # Scattered Showers 7 [ATMOS-X], precipitationPower 0.35
}
