...
* **[Options]** New Campaign Management options "Maximum auto-planned packages per turn" (separate values for the player and the enemy) cap how many packages the auto-planner creates each turn, in any campaign. 0 (the default) keeps the current unlimited behaviour; missions are kept in the planner's usual priority order.
