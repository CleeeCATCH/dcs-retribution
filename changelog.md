...
* **[Options]** New "Air-to-Air Missiles" settings page lets you choose the A2A missiles every aircraft carries, instead of the missiles pinned by each aircraft's default loadouts. Rank radar-guided and infrared-guided missiles (separately for the player and the enemy), and every pylon that carries a ranked missile is re-armed with the highest ranked missile that pylon can carry. For example, ranking AIM-120D-3 above AIM-120C gives AIM-120Ds to every aircraft that normally carries AIM-120Cs and can take the AIM-120D. Racks keep their missile count, unranked missiles and custom loadouts are never changed, and missiles not yet in service are skipped when weapons are restricted by date. Every new campaign starts with the default loadout missiles, and edits only take effect once saved with "Save Changes".
...
* **[Performance]** New option "Put idle ground units to sleep" switches off the AI of ground units at battle positions, motorpools and strike targets while no enemy ground unit, enemy helicopter or player is within a configurable wake radius, and no airborne enemy airplane is within a separate, smaller airplane wake radius. Sleeping units can still be seen, attacked and destroyed, and wake up for good as soon as they are hit. Air defences, ships, artillery and missile launchers are never put to sleep.
* **[Options]** New Campaign Management options "Maximum auto-planned packages per turn" (separate values for the player and the enemy) cap how many packages the auto-planner creates each turn, in any campaign. BARCAP, AEW&C and tanker packages don't count toward the limit. 0 (the default) keeps the current unlimited behaviour; missions are kept in the planner's usual priority order.
